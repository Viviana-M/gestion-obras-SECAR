{{-- ═══════════ PESTAÑA: SEGURIDAD SOCIAL POR PERSONA ═══════════ --}}
@php $maxTotal = (float) ($personas->max('total') ?? 0); @endphp
<div id="tab-personas" style="display:none">

    {{-- Filtros de la pestaña: UN + buscador + Excel --}}
    <div class="card" style="padding:12px 16px;margin-bottom:1rem;display:flex;gap:12px;align-items:flex-end;flex-wrap:wrap">
        <form method="GET" action="{{ route('contable.autoliquidacion.index') }}" style="margin:0">
            ...
            <input type="hidden" name="tab" id="tab-input-un" value="personas">
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px;margin-bottom:1rem">
        <div class="card" style="padding:14px 16px">
            ...
            <div style="font-size:22px;font-weight:700;color:#15803D">{{ $fmt($total) }}</div>
        </div>
        <div class="card" style="padding:14px 16px">
    ...
    <div class="ss-layout" style="display:grid;grid-template-columns:minmax(300px,400px) 1fr;gap:16px;align-items:start">
        ...
        {{-- Lista de PERSONAS (por nombre), mayor a menor --}}
        ...
        {{-- Detalle de la persona: dona por concepto --}}
...
const fmtSS = n => '$' + Math.round(n).toLocaleString('es-CO');
...
function filtrar(){
    ...
    const vac = document.getElementById('ss-sinresultados');
    if (vac) vac.style.display = visibles ? 'none' : 'block';
}
...
function seleccionar(i){
    ...
    document.querySelectorAll('#ss-lista .ss-item').forEach(el => el.style.background = (el.dataset.i == i) ? '#EFF6FF' : '');
    ...
    document.getElementById('d-total').textContent = fmtSS(p.total);
    ...
    d.items.forEach(function(c, idx){
        ...
        filas += `<tr style="border-top:1px solid #F3F4F6">
            ...
            <td style="padding:5px 6px;text-align:right;font-weight:600;color:#15803D">${fmtSS(c.aporte)}</td>
