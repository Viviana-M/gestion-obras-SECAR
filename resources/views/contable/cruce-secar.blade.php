...
@section('content')
<x-page-banner title="Cruce cuenta 14 vs tercero SECAR" icon="🔎">
    Saldo de la cuenta 14 ("Costos por aplicar") por obra, separando lo que está contra el propio
    tercero <b>SECAR</b> (reversiones que distorsionan el saldo) de lo <b>real</b> contra terceros.
    <b>Saldo real = Saldo total − Saldo SECAR</b>.
    <x-slot:actions>
    ...
    <table style="width:100%;border-collapse:collapse;font-size:13px;min-width:820px">
        <thead>
            <tr style="background:#1B3F6E;color:white">
                ...
                <th style="padding:10px 14px;text-align:right">Saldo total</th>
                <th style="padding:10px 14px;text-align:right">Saldo real (terceros)</th>
            </tr>
        </thead>
        <tbody>
            @forelse($filas as $f)
            <tr style="border-bottom:1px solid #E5E7EB">
                ...
                <td style="padding:9px 14px;text-align:right;color:{{ $col($f['saldo_total']) }}">{{ $fmt($f['saldo_total']) }}</td>
                <td style="padding:9px 14px;text-align:right;font-weight:700;color:{{ $col($f['saldo_real']) }}">{{ $fmt($f['saldo_real']) }}</td>
            </tr>
            @empty
            <tr><td colspan="6" style="padding:1.5rem;text-align:center;color:#9CA3AF">No hay obras con saldo contra SECAR en la cuenta 14. 🎉</td></tr>
            @endforelse
        </tbody>
        @if(count($filas))
        <tfoot>
            <tr style="border-top:2px solid #E5E7EB;background:#F9FAFB;font-weight:700">
                ...
                <td style="padding:10px 14px;text-align:right;color:{{ $col($totalGeneral) }}">{{ $fmt($totalGeneral) }}</td>
                <td style="padding:10px 14px;text-align:right;color:{{ $col($totalReal) }}">{{ $fmt($totalReal) }}</td>
            </tr>
        </tfoot>
        @endif
    </table>
